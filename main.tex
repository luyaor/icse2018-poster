\documentclass[sigconf]{acmart}


\newcommand{\Luyao}[1]{\textcolor{red}{(Luyao: #1)}}
\newcommand{\tocheck}[1]{\textcolor{orange}{(Luyao: #1)}}
\newcommand{\Shurui}[1]{\textcolor{blue}{(Shurui: #1)}}
\newcommand{\CK}[1]{\textcolor{purple}{(CK: #1)}}
\newcommand{\Andrzej}[1]{\textcolor{magenta}{(Andrzej: #1)}}
\graphicspath{{./figures/}}
\newcommand{\hlc}[2][pink]{{\sethlcolor{#1}\hl{#2}}}
\usepackage{todonotes}
\usepackage{booktabs} % For formal tables
\usepackage{float}

% Copyright
\setcopyright{acmcopyright}
\settopmatter{printacmref=false} % Removes citation information below abstract
\renewcommand\footnotetextcopyrightpermission[1]{} % removes footnote with conference information in first column
\pagestyle{plain} % removes running headers
% DOI
\acmDOI{10.475/123_4}

% ISBN
\acmISBN{123-4567-24-567/08/06}

%Conference
\acmConference[ICSE'2018]{ACM International Conference on Software Engineering}{May 2018}{Goteborg, Sweden} 
\acmYear{2018}
\copyrightyear{2018}

\acmPrice{15.00}

\begin{document}
\title{Poster: Forks Insight: Providing an Overview of GitHub Forks}

	\author{Luyao Ren}
	\affiliation{
		\institution{Peking University}
		%\country{China}
	}

	\author{Shurui Zhou}
	\affiliation{
		\institution{Carnegie Mellon University}
		%\country{USA}
	}
	
	\author{Christian K\"{a}stner} 
	\affiliation{
		\institution{Carnegie Mellon University}
		%\country{USA}
	}

	\iffalse
       \author{Andrzej W\k{a}sowski}
	\affiliation{%
		\institution{IT University of Copenhagen}
		%\country{Denmark}
	}
	\fi


	% The default list of authors is too long for headers}
	\renewcommand{\shortauthors}{L. Ren et al.}
	

\begin{abstract}
\Luyao{TODO} 
Fork-based development allows developers to start development from existing software repository by copying the code files. We implemented Forks Insight (\url{http://www.forks-insight.com}) to help developers get an overview of forks on GitHub. The current release version focuses on simple analytics for the high level overview which is lightweight, scalable and practical. It has an user-friendly interactive web interface with features for searching and tagging.

\end{abstract}

\keywords{\Luyao{TODO} Fork, Overview, GitHub}


\maketitle

\input{body}
\bibliographystyle{ACM-Reference-Format}
\bibliography{references} 

\end{document}
